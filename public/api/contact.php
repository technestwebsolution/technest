...
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    header('Allow: POST');
    respond(false, 'method_not_allowed', 'Only POST requests are allowed.', 405);
}

$projectRoot = locateProjectRoot();
if ($projectRoot === null) {
    error_log('[technest-contact] Composer autoload file was not found. Run composer install.');
    respond(false, 'server_error', 'The mail service is not configured yet.', 500);
}
...
if (is_string($origin) && $origin !== '' && is_string($host) && $host !== '') {
    ...
    if (!is_string($originHost) || !hash_equals($requestHost, strtolower($originHost))) {
        respond(false, 'forbidden', 'This form request was rejected.', 403);
    }
}
